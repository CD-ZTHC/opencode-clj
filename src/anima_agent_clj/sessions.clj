(ns anima-agent-clj.sessions
  "Session management for opencode-server.

   Functions: list-sessions, create-session, get-session, update-session,
...
(defn init-session
  ...
  (-> (http/post-request client (str "/session/" (http/session-id session-id) "/init")
...
(defn summarize-session
  ...
  (utils/validate-required {:providerID providerID :modelID modelID}
                           [:providerID :modelID])
  (-> (http/post-request client (str "/session/" (http/session-id session-id) "/summarize")
